...
std::vector<T> apply_matrix(const MT& matrix, const std::vector<T>& in, bool dagger = false) {
...
void make_plane_wave(double t, size_t n, std::vector<double>& ev, std::vector<std::vector<std::complex<double>>>& u) {
  using namespace std;
  ...
  sort(ke.begin(), ke.end(), [](const auto& x, const auto& y) { return x.second < y.second; });
