...
private:
  ...
  real_t<T> find_mth_eigenvalue(const std::vector<real_t<T>>&,
...
template <typename T>
inline LambdaLanczos<T>::LambdaLanczos(std::function<void(const std::vector<T>&, std::vector<T>&)> mv_mul,
...
template <typename T>
inline int LambdaLanczos<T>::run(real_t<T>& eigvalue, std::vector<T>& eigvec) const {
...
template <typename T>
inline void LambdaLanczos<T>::schmidt_orth(std::vector<T>& uorth, const std::vector<std::vector<T>>& u) {
...
template <typename T>
inline util::real_t<T> LambdaLanczos<T>::find_mth_eigenvalue(const std::vector<util::real_t<T>>& alpha,
 ...
 */
template <typename T>
inline util::real_t<T> LambdaLanczos<T>::tridiagonal_eigen_limit(const std::vector<real_t<T>>& alpha,
...
template <typename T>
inline void VectorRandomInitializer<T>::init(std::vector<T>& v) {
...
template <typename T>
inline void VectorRandomInitializer<std::complex<T>>::init(std::vector<std::complex<T>>& v) {
  std::random_device dev;
  std::mt19937 mt(dev());
  std::uniform_real_distribution<T> rand((T)(-1.0), (T)(1.0));
